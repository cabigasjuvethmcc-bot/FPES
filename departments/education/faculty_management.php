...
// Load faculty list for Education
try {
...
<head>
  ...
  <title>Education Department - Manage Faculty</title>
  <link rel="stylesheet" href="../../styles.css">
  <link rel="stylesheet" href="education-theme.css">
  <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css" rel="stylesheet">
  <style>
    :root { --education-primary: #1e3a8a; --education-secondary: #3b82f6; }
    ...
    .education-btn { background: var(--education-primary); color: #fff; padding: 10px 14px; border-radius: 10px; text-decoration: none; border: none; cursor: pointer; }
    ...
    .education-table th, .education-table td { padding: 12px; border-bottom: 1px solid #e5e7eb; text-align: left; }
  </style>
</head>
<body>
  <div class="dashboard">
    ...
    <div class="main-content">
      <div style="padding: 20px; max-width: 1200px; margin: 0 auto;">
        <div style="display:flex; justify-content: space-between; align-items:center; margin-bottom:16px;">
          <h1 style="margin:0;">Manage Faculty - Education</h1>
        </div>
        ...
        <div style="background:#fff; border-radius:12px; padding:16px; box-shadow: var(--card-shadow);">
            ...
            <tbody>
              <?php foreach ($faculty_list as $f): ?>
                <tr>
                  ...
                  <td>
                    <button class="education-btn" onclick="openEditSubjects(<?php echo (int)$f['id']; ?>, '<?php echo htmlspecialchars($f['full_name'] ?? $f['username'], ENT_QUOTES); ?>')">
    ...
    <div style="position:absolute; top:50%; left:50%; transform: translate(-50%, -50%); background:#fff; padding:24px; border-radius:12px; width:90%; max-width:700px;">
      ...
      <form method="POST" id="subjectsForm">
        ...
        <div class="form-group">
          <label>Subjects (Education)</label>
          <div class="subject-picker">
            <div class="picker-toolbar" style="display:flex; gap:8px; align-items:center; margin:6px 0;">
              ...
              <button type="button" class="education-btn" id="btnSelectAll" style="background:#16a34a;">Select All</button>
              <button type="button" class="education-btn" id="btnClear" style="background:#ef4444;">Clear</button>
            </div>
            ...
            <small style="color:#666; display:block; margin-top:6px;">Tip: Hold Ctrl (Windows) to select multiple subjects.</small>
          </div>
        </div>
        <div style="display:flex; gap:10px; justify-content:flex-end; margin-top: 12px;">
          <button type="button" class="education-btn" style="background:#6b7280;" onclick="closeModal()">Cancel</button>
          <button type="submit" class="education-btn">Save</button>
        </div>
      </form>
    </div>
  </div>
  ...
  <script>
    async function fetchSubjects() {
      try {
        const r = await fetch(`../../api/subjects.php?department=Education`);
    ...
    async function fetchFacultySubjects(uid) {
      try {
        const r = await fetch(`../../api/faculty_subjects.php?faculty_user_id=${encodeURIComponent(uid)}`);
        const d = await r.json();
        return (d && d.success && Array.isArray(d.data)) ? d.data : [];
      } catch { return []; }
    }
