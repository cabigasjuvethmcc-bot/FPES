...
if (!$evalId) {
    ...
    echo 'Self-evaluation not found.';
    exit;
}

// Load self-evaluation ensuring it belongs to the logged-in faculty
$evaluation = null;
try {
...
if (!$evaluation) {
    http_response_code(404);
    echo 'Self-evaluation not found or access denied.';
    exit;
}

// Load criteria responses for self-evaluation
$responses = [];
try {
    $stmt = $pdo->prepare("SELECT ec.category, ec.criterion, ser.rating, ser.comment
                           FROM self_evaluation_responses ser
                           JOIN evaluation_criteria ec ON ser.criterion_id = ec.id
                           WHERE ser.self_eval_id = ?
...
<head>
    ...
    <title>Self-Evaluation #<?php echo htmlspecialchars((string)$evalId); ?> - Details</title>
    ...
    <div class="container">
        ...
        <div class="meta" style="margin-top:8px;">
            <div class="meta-item" style="grid-column:1/-1;">
                ...
                <span><?php echo nl2br(htmlspecialchars($evaluation['overall_comments'])); ?></span>
            </div>
        </div>
        <?php endif; ?>
